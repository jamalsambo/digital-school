    ...
    <div class="page-topbar row items-center q-px-xl q-py-md">
      ...
      <div class="col">
        <q-breadcrumbs class="breadcrumb-nav" active-color="primary" separator="›">
          ...
          <q-breadcrumbs-el label="Fornecedores" class="breadcrumb-active" />
        </q-breadcrumbs>

        <div class="page-title row items-center q-mt-xs">
          <div class="title-icon-wrap q-mr-sm">
            <q-icon name="local_shipping" size="1.3rem" color="white" />
          </div>
      ...
      <div class="col-auto row items-center q-gutter-sm">
        ...
        <!-- Refresh -->
        <q-btn flat round dense icon="refresh" class="action-icon-btn" @click="fetchSuppliers" />
        ...
        <q-card-section class="q-pa-none">
          ...
          <div class="table-toolbar row items-center justify-between q-px-lg q-py-md">
            <div class="row items-center q-gutter-xs">
              <span class="table-count">{{ filteredSuppliers.length }}</span>
              <span class="table-count-label">fornecedores encontrados</span>
            </div>
            <div class="row items-center q-gutter-xs">
              <q-btn
          ...
          <div v-else class="grid-view q-pa-lg row q-gutter-md">
            ...
            >
              <div class="grid-card-header">
                <q-avatar size="48px" class="supplier-avatar">
                  <q-icon name="local_shipping" color="white" />
                </q-avatar>
              </div>

              <div class="grid-card-body">
                <div class="grid-name">{{ supplier.name }}</div>
                <div class="grid-info row items-center q-gutter-xs q-mt-xs">
                  <q-icon name="phone" size="0.9rem" color="grey-5" />
                  <span>{{ supplier.phone ?? '—' }}</span>
                </div>
                <div class="grid-info row items-center q-gutter-xs q-mt-xs" v-if="supplier.email">
                  <q-icon name="email" size="0.9rem" color="grey-5" />
...
const filteredSuppliers = computed(() => {
  if (!filter.value) return suppliers.value;
...
const stats = computed(() => [
  ...
  {
    label: "Activos",
    value: suppliers.value.filter((s) => s.status === "Activo").length,
    icon: "check_circle",
    color: "#21b573",
  },
  {
...
const editSupplier = (supplier) => {
  router.push({ name: "stock-supplier-edit", params: { id: supplier.id } });
};
...
const fetchSuppliers = async () => {
  try {
    ...
    notifyError("Erro ao carregar fornecedores");
  }
};
