    ...
    <div class="page-topbar row items-center q-px-xl q-py-md">
      ...
      <div class="col">
        <q-breadcrumbs class="breadcrumb-nav" active-color="primary" separator="›">
          ...
          <q-breadcrumbs-el :label="title" class="breadcrumb-active" />
        </q-breadcrumbs>
        ...
        <q-card-section class="q-pa-none">
          ...
          <div class="table-toolbar row items-center justify-between q-px-lg q-py-md">
            <div class="row items-center q-gutter-xs">
              <span class="table-count">{{ filteredRooms.length }}</span>
              <span class="table-count-label">{{ title.toLowerCase() }} encontradas</span>
            </div>
          ...
          <div v-else class="grid-view q-pa-lg row q-gutter-md">
            ...
            >
              <div class="grid-card-header">
                <q-avatar size="48px" class="room-avatar">
                  <q-icon name="meeting_room" color="white" />
                </q-avatar>
              </div>

              <div class="grid-card-body">
                <div class="grid-name">{{ room.name }}</div>
                <div class="grid-info">Capacidade: {{ room.capacity ?? '—' }}</div>
              </div>
...
const stats = computed(() => [
  ...
  {
    label: "Activas",
    value: rooms.value.filter((r) => r.status === "Activo").length,
    icon: "check_circle",
    color: "#21b573",
  },
  {
...
const editRoom = (row) => {
  router.push({ name: "edit-room", params: { id: row.id } });
};
...
.grid-name { font-family: "Sora", sans-serif; font-weight: 600; font-size: 0.95rem; color: #0f2862; margin-bottom: 2px; }
