    ...
    <div class="page-topbar row items-center q-px-xl q-py-md">
      ...
      <div class="col">
        <q-breadcrumbs class="breadcrumb-nav" active-color="primary" separator="›">
          ...
          <q-breadcrumbs-el :label="`${getNameForDiscipline(module)}s`" class="breadcrumb-link" @click="$router.back()" style="cursor:pointer" />
        ...
        <div class="page-title row items-center q-mt-xs">
          <div class="title-icon-wrap q-mr-sm">
            <q-icon name="menu_book" size="1.3rem" color="white" />
          </div>
          <span>{{ pageTitle }}</span>
        </div>
      </div>
      ...
      <q-card class="form-card" flat>
        ...
        <div class="form-card-header row items-center q-px-lg q-py-md">
          <div class="form-header-icon q-mr-md">
            <q-icon name="menu_book" size="1.2rem" color="white" />
          </div>
          <div>
            <div class="form-header-title">{{ pageTitle }}</div>
            <div class="form-header-sub">
              Preencha os campos abaixo para {{ moduleId ? 'actualizar' : 'registar' }} a {{ getNameForDiscipline(module).toLowerCase() }}
            </div>
          </div>
        </div>
        ...
        <q-card-section class="q-pa-lg">
          <q-form ref="formRef" @submit.prevent="submitForm">
            ...
            <div class="section-label q-mb-md">Informações da {{ getNameForDiscipline(module) }}</div>

            <div class="row q-col-gutter-md q-mb-lg">
              <!-- Nome -->
              <div class="col-12">
                <div class="field-label">Nome da {{ getNameForDiscipline(module) }} <span class="required">*</span></div>
                <q-input
                  v-model="formData.name"
                  :placeholder="`Ex: ${getNameForDiscipline(module)} de Matemática`"
                  outlined dense
                  class="edigital-input"
                  :rules="[(val) => !!val || `O nome da ${getNameForDiscipline(module).toLowerCase()} é obrigatório.`]"
                />
              </div>
            </div>
            ...
            <div class="row justify-end q-gutter-sm">
              ...
              />
              <q-btn
                :label="moduleId ? 'Actualizar' : 'Guardar'"
                icon="save"
                class="save-btn"
                no-caps
                type="submit"
...
const pageTitle = computed(() =>
  moduleId
    ? `Editar ${getNameForDiscipline(module)}`
    : `Criar ${getNameForDiscipline(module)}`
);
...
const submitForm = async () => {
  ...
  try {
    if (!moduleId) {
      ...
      notifySuccess(`${getNameForDiscipline(module)} actualizada com sucesso!`);
    }
    router.back();
  } catch (error) {
    notifyError(`Erro ao guardar ${getNameForDiscipline(module).toLowerCase()}`);
  } finally {
