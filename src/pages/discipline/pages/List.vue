  ...
  <q-page class="edigital-page">
    ...
    <div class="page-topbar row items-center q-px-xl q-py-md">
      ...
      <div class="col-auto row items-center q-gutter-sm">
        ...
        <!-- Refresh -->
        <q-btn flat round dense icon="refresh" class="action-icon-btn" @click="fetchDisciplines" />

        <!-- Add -->
        <q-btn
          unelevated
          icon="add"
          :label="`Adicionar ${getNameForDiscipline(moduleUpdated)}`"
          class="new-btn"
          no-caps
        ...
        <q-card-section class="q-pa-none">
          ...
          <div class="table-toolbar row items-center justify-between q-px-lg q-py-md">
            <div class="row items-center q-gutter-xs">
              <span class="table-count">{{ filteredDisciplines.length }}</span>
              <span class="table-count-label">{{ pageTitle.toLowerCase() }} encontradas</span>
            </div>
            <div class="row items-center q-gutter-xs">
              <q-btn
...
/* ── Computed ── */
const pageTitle = computed(() => `${getNameForDiscipline(moduleUpdated.value)}s`);

const filteredDisciplines = computed(() => {
  if (!filter.value) return disciplines.value;
...
const stats = computed(() => [
  {
    label: "Total",
    value: disciplines.value.length,
    icon: "menu_book",
    color: "#1a3fa6",
  },
  {
    label: "Activas",
    value: disciplines.value.filter((d) => d.status === "Activo").length,
    icon: "check_circle",
    color: "#21b573",
  },
  {
...
const deleteDiscipline = async (discipline) => {
  try {
    ...
    notifySuccess("Eliminado com sucesso!");
...
const fetchDisciplines = async () => {
  try {
    ...
    notifyError("Erro ao buscar disciplinas");
  }
};
