    ...
    <div class="page-topbar row items-center q-px-xl q-py-md">
      ...
      <div class="col-auto row items-center q-gutter-sm">
        ...
        <q-input
          v-model="filter"
          dense outlined
          placeholder="Pesquisar patrimónios..."
          class="search-input"
          debounce="300"
          clearable
        >
        ...
        <q-card-section class="q-pa-none">
          ...
          <div class="table-toolbar row items-center justify-between q-px-lg q-py-md">
            <div class="row items-center q-gutter-xs">
              <span class="table-count">{{ filteredAssets.length }}</span>
              <span class="table-count-label">de {{ assets.length }} património(s)</span>
            </div>
            <div class="row items-center q-gutter-xs">
              <q-btn
          ...
          <div v-else-if="!filteredAssets.length" class="empty-state">
            <div class="empty-icon-wrap q-mb-md">
              <q-icon name="domain" size="2.5rem" color="white" />
            </div>
            <div class="empty-title">Nenhum património encontrado</div>
            <div class="empty-sub">
              {{ filter ? 'Tente outro termo de pesquisa' : 'Clique em "Adicionar Património" para começar' }}
            </div>
            <q-btn
              v-if="!filter"
            ...
            <template #body-cell-actions="props">
              <q-td :props="props">
                <div class="row no-wrap items-center q-gutter-xs">
                  ...
                  <q-btn
                    flat round dense icon="compare_arrows"
                    class="tbl-btn tbl-btn-orange"
          ...
          <div v-else class="grid-view q-pa-lg row q-gutter-md">
            ...
            >
              <div class="grid-card-header row items-center justify-between">
                <q-avatar size="48px" class="asset-avatar">
                  <q-icon name="domain" color="white" />
                </q-avatar>
                <q-chip
                  dense
                  :class="asset.status === 'Activo' || asset.status === 'active'
                    ? 'chip-activo' : 'chip-movimento'"
                >
                  {{ asset.status }}
                </q-chip>
              </div>
              ...
              <div class="grid-card-body q-mt-sm">
                ...
                <div class="grid-info row items-center q-gutter-xs q-mt-xs">
                  <q-icon name="tag" size="0.9rem" color="grey-5" />
              ...
              <div class="grid-card-actions row q-gutter-xs">
                ...
                />
                <q-btn
                  flat dense no-caps icon="compare_arrows" label="Mover"
                  class="grid-action-btn grid-action-orange"
                  @click="createMove(asset)"
                />
              </div>
            </div>
          </div>
...
const stats = computed(() => [
  ...
  },
  {
    ...
    value: assets.value.filter(
      (a) => a.status === "Activo" || a.status === "active"
    ).length,
    icon: "check_circle",
    color: "#21b573",
  },
  {
    label: "Em Movimento",
    value: assets.value.filter(
      (a) => a.status === "Em movimento" || a.status === "moved"
    ).length,
    icon: "compare_arrows",
    color: "#f57c00",
  },
...
const fetchAssets = async () => {
  ...
  try {
    ...
    notifyError("Erro ao carregar patrimónios");
  } finally {
    loading.value = false;
  }
};
...
.stat-card:hover { box-shadow: 0 4px 20px rgba(15, 40, 98, 0.1); transform: translateY(-2px); }
.stat-icon {
  width: 44px;
  height: 44px;
  border-radius: 12px;
...
.grid-action-orange { color: #f57c00 !important; background: #fff3e0 !important; }
