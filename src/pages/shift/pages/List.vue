    ...
    <div class="page-topbar row items-center q-px-xl q-py-md">
      ...
      <div class="col">
        <q-breadcrumbs class="breadcrumb-nav" active-color="primary" separator="›">
          ...
          <q-breadcrumbs-el label="Horários" class="breadcrumb-active" />
        </q-breadcrumbs>
      ...
      <q-card class="table-card" flat>
        <q-card-section class="q-pa-none">
          ...
          <div v-else class="grid-view q-pa-lg row q-gutter-md">
            <div
              v-for="shift in filteredRows"
              :key="shift.id"
              class="grid-card col-12 col-sm-6 col-md-4 col-lg-3"
            >
              <div class="grid-card-header">
                <q-avatar size="48px" class="shift-avatar">
                  <q-icon name="schedule" color="white" />
                </q-avatar>
              </div>

              <div class="grid-card-body">
                <div class="grid-name">{{ shift.name }}</div>
                <div class="grid-info row items-center q-gutter-xs q-mt-xs">
                  <q-icon name="access_time" size="0.9rem" color="grey-5" />
...
const title = ref("Horários");

/* ── Computed ── */
const filteredRows = computed(() => {
  if (!filter.value) return rows.value;
  const q = filter.value.toLowerCase();
  return rows.value.filter((s) => s.name?.toLowerCase().includes(q));
});
...
const stats = computed(() => [
  ...
  {
    label: "Manhã",
    value: rows.value.filter((s) => s.period === "Manhã").length,
    icon: "wb_sunny",
    color: "#f59e0b",
  },
  {
    label: "Tarde",
    value: rows.value.filter((s) => s.period === "Tarde").length,
    icon: "nights_stay",
    color: "#6366f1",
  },
]);
...
const addShift = () => {
  router.push({ name: "create-period" });
};

const editShift = (row) => {
  router.push({ name: "edit-period", params: { id: row.id } });
};
