    ...
    <div class="page-topbar row items-center q-px-xl q-py-md">
      ...
      <div class="col">
        <q-breadcrumbs class="breadcrumb-nav" active-color="primary" separator="›">
          ...
          <q-breadcrumbs-el label="Criar Horário" class="breadcrumb-active" />
        </q-breadcrumbs>

        <div class="page-title row items-center q-mt-xs">
          <div class="title-icon-wrap q-mr-sm">
            <q-icon name="schedule" size="1.3rem" color="white" />
          </div>
      ...
      <q-card class="form-card" flat>
        ...
        <div class="form-card-header row items-center q-px-lg q-py-md">
          <div class="form-header-icon q-mr-md">
            <q-icon name="schedule" size="1.2rem" color="white" />
          </div>
          <div>
            <div class="form-header-title">Criar Horário</div>
            <div class="form-header-sub">Defina o período de entrada e saída do horário</div>
          </div>
        </div>
        ...
        <q-card-section class="q-pa-lg">
          <q-form ref="formRef" @submit.prevent="handleSubmit">
            ...
            <div class="section-label q-mb-md">Período do Horário</div>

            <div class="row q-col-gutter-md q-mb-lg">

              <!-- Entrada -->
              <div class="col-md-6 col-12">
                <div class="field-label">Hora de Entrada <span class="required">*</span></div>
                <q-input
                  v-model="form.start"
                  mask="##:##"
                  ...
                  :rules="[(val) => !!val || 'A hora de entrada é obrigatória.']"
                >
                  <template #append>
              ...
              <!-- Saída -->
              <div class="col-md-6 col-12">
                <div class="field-label">Hora de Saída <span class="required">*</span></div>
                <q-input
                  v-model="form.end"
                  mask="##:##"
                  ...
                  outlined dense
                  class="edigital-input"
                  :rules="[(val) => !!val || 'A hora de saída é obrigatória.']"
                >
                  <template #append>
                    <q-icon name="access_time" class="cursor-pointer time-icon">
                      <q-popup-proxy>
            ...
            <!-- Preview do horário -->
            <div class="shift-preview row items-center q-gutter-sm q-mb-lg" v-if="form.start || form.end">
              <q-icon name="schedule" color="primary" size="1.1rem" />
              <span class="preview-label">Horário:</span>
              <q-chip dense class="shift-chip">
                {{ form.start || '--:--' }} — {{ form.end || '--:--' }}
              </q-chip>
            </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    notifySuccess("Horário criado com sucesso!");
    router.back();
  } catch (error) {
    notifyError("Erro ao criar horário");
  } finally {
    loading.value = false;
  }
};
