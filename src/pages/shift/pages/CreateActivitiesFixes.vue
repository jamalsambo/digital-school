    ...
    <div class="page-topbar row items-center q-px-xl q-py-md">
      ...
      <div class="col">
        <q-breadcrumbs class="breadcrumb-nav" active-color="primary" separator="›">
          ...
          <q-breadcrumbs-el label="Criar Actividade Fixa" class="breadcrumb-active" />
        </q-breadcrumbs>

        <div class="page-title row items-center q-mt-xs">
          <div class="title-icon-wrap q-mr-sm">
            <q-icon name="push_pin" size="1.3rem" color="white" />
          </div>
          <span>Criar Actividade Fixa</span>
        </div>
      </div>
    </div>
      ...
      <q-card class="form-card" flat>
        ...
        <div class="form-card-header row items-center q-px-lg q-py-md">
          <div class="form-header-icon q-mr-md">
            <q-icon name="push_pin" size="1.2rem" color="white" />
          </div>
          <div>
            <div class="form-header-title">Criar Actividade Fixa</div>
            <div class="form-header-sub">Preencha o campo abaixo para registar a actividade fixa</div>
          </div>
        </div>
        ...
        <q-card-section class="q-pa-lg">
          <q-form ref="formRef" @submit.prevent="handleSubmit">
            ...
            <div class="section-label q-mb-md">Informações da Actividade</div>

            <div class="row q-col-gutter-md q-mb-lg">
              <!-- Nome / Label -->
              <div class="col-12">
                <div class="field-label">Nome da Actividade <span class="required">*</span></div>
                <q-input
                  v-model="name"
                  type="text"
                  ...
                  :rules="[(val) => !!val || 'O nome da actividade é obrigatório.']"
                />
              </div>
            </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    notifySuccess("Actividade Fixa criada com sucesso!");
    router.back();
  } catch (error) {
    notifyError("Erro ao criar actividade fixa");
  } finally {
    loading.value = false;
  }
};
